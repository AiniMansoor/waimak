...
class ModelTools(object):
    ...
    def plt_matrix(self, array=None, vmin=None, vmax=None, title=None, no_flow_layer=0, ax=None, color_bar=True,
                   base_map=False, plt_background=True, cbar_lab=None, contour_array = None, **kwargs):
        """
        ...
        if base_map:
            ...
            ax.imshow(image, extent=[ll[0], ur[0], ll[1], ur[1]], cmap='gray', alpha=alpha) #
        ...
        if type(contour_array) is np.ndarray:
            pc = ax.contour(model_xs, model_ys, contour_array, levels=np.linspace(0., 5.,11), colors="red", linewidth = 1.5)
            plt.clabel(pc, fmt="%2.1f", fontsize="x-small")
